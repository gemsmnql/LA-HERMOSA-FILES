    ...
    <div class="mx-[120px] mt-[20px] flex gap-[50px] main-content-wrapper">
        <div class="w-3/4 left-column">
            ...
            <div *ngIf="!isLoading && blogPosts.length > 0" class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-8 responsive-grid animate-in fade-in duration-700">
                <article *ngFor="let article of blogPosts" [routerLink]="['/blogs', article.slug || article._id || article.id]"
                    ...
                    <img [src]="getImageUrl(article.imageUrl)" [alt]="article.altText || ''" 
                         class="w-full h-[420px] object-cover transition duration-500 group-hover:scale-110"
                         onerror="this.src='https://placehold.co/400x600?text=La+Hermosa'">
                    ...
                    <div class="card-details absolute bottom-0 p-6 text-white bg-gradient-to-t from-black/80 to-transparent w-full">
                        <h3 class="text-[30px] leading-tight">{{ article.title }}</h3>
                        <p class="text-xs">{{ article.createdAt | date:'MM/dd/yyyy' }}</p>
                        <p class="text-sm mt-2 opacity-90 line-clamp-2">{{ article.content }}</p>
                    </div>
                </article>
            </div>
        </div>
        ...
        <div class="w-1/4 space-y-10 sidebar-column">
            <div *ngIf="!isLoading && featuredPosts.length > 0" class="space-y-6 animate-in fade-in duration-700">
                <div *ngFor="let item of featuredPosts" [routerLink]="['/blogs', item.slug || item._id || item.id]"
                     class="group flex gap-4 p-3 rounded-xl transition hover:bg-gray-100 cursor-pointer">
                    <img [src]="getImageUrl(item.imageUrl)" [alt]="item.altText || ''" 
            ...
            <div *ngIf="!isLoading && latestPosts.length > 0" class="animate-in fade-in duration-700">
                <div class="right-article flex items-center gap-[10px] mb-6">
                    <h2 class="text-[40px] whitespace-nowrap">Latest</h2>
                    <div class="h-[2px] bg-gray-300 w-full ml-6"></div>
                </div>
                <div class="space-y-6">
                    <div *ngFor="let item of latestPosts" [routerLink]="['/blogs', item.slug || item._id || item.id]"
